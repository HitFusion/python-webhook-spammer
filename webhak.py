...
def main():
    print("webhook bomber by HitFusion (0 effort tysm YeezyDaOne for adding threads)")
    webhook_url = input("webhook link: ")
    message = input("thing u want to send: ")
    count = int(input("number of messages you want: "))

    usetheshit = input("Do you want to use threading? (yes/no) (Shit will prank): ").strip().lower()
    if usetheshit == 'yes':
